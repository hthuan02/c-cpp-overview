...
{   
    ...
    create_node(&node1, 5)  // 0x01 (NULL, 5)
    create_node(&node2, 34) // 0xa1 (NULL, 34)
    create_node(&node2, 12) // 0xe1 (NULL, 12)

    return 0;
}
```
- Nhưng với danh sách liên kết thì sẽ lưu rất nhiều phần tử, kích thước mỗi Node là kích thước của `Struct Node` ngốn rất nhiều bộ nhớ. 
- Để khắc phục thì khởi tạo linked-list lưu ở phân vùng `Heap`. Số lượng vùng nhớ trên `Heap` >> `Stack`

#### Cách 2: Cấp phát động vùng nhớ `Heap` trước, đẩy các thông tin & dữ liệu vào vùng nhớ `Heap`. Khi sử dụng thì dùng con trỏ để trỏ đến vùng nhớ đấy.
...
{
    // Cấp phát động bộ nhớ cho node đầu tiên trước
    Node *node = (Node*)malloc(newData * sizeof(Node));
    // Sau đó khởi tạo dữ liệu & con trỏ NULL
    node->next = NULL;
    node->data = newData;

    // Vì là kiểu node, nên return node
    return node;
}

int main ()
{   
    // Vì kiểu dữ liệu là con trỏ kiểu Node
    Node *node1, *node2, *node3;

    *node1 = create_node(5);    //0xa1
    *node1 = create_node(6);    //0xb1
    *node1 = create_node(6);    //0x.. khác địa chỉ trên 
...
**LƯU Ý QUAN TRỌNG:**
- Trong stack, queue phần tử không nhiều nên thông thường chỉ cấp phát động mảng. Còn các giá trị top(trong stack), giá trị (front/rear trong queue) vẫn trong stack.

```c
void stack_create(Stack *stack, int newData)
{
    stack->items = (int*)malloc(newData * sizeof(int)); // Chỉ cấp phát động cho mảng trong heap

    // Còn lại, truyền địa chỉ cục bộ trong stack
    stack-> size = newData;
    stack-> top = -1;
}
```

- Trong linked-list các số phần tử là rất lớn, có thể lên 10000 phần tử. Nên cấp phát động **toàn bộ Node** bao gồm: dữ liệu & con trỏ trỏ đến node tiếp theo.

--> Khi gọi thì dùng con trỏ để gọi
```c
// Cấp phát toàn bộ trong Heap 
Node* create_node(int newData)
{
...
int main()
{   
    Node *node1, *node2; // Dùng  con trỏ để gọi ra
    ....
...
```
#### CHỐT LẠI:
Cách 2 khác cách 1 3 điểm
- Cách 2 có 1 tham số truyền vài, cách 1 có 2 tham số.
- Cách 2 kiểu trả về là 1 con trỏ node, cách 1 kiểu trả về là kiểu dữ liệu bình thường.
